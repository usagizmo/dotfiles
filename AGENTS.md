...
| 置く場所 | 対象 | 判定 |
|---------|------|------|
| `agents/` | instructions / rules / skills | 2 つ以上の harness で同じ意味・手順を使いたい。本文から harness 名・固有 API を消せる。`~/.agents/` にも投影する |
| `harnesses/shared/skills` | harness skills 入口だけに配る共有 skill | 複数 harness で同じ中身だが、`agents/skills`（`~/.agents/skills` 投影）には載せないもの。`inv_harness_skills` 経由でオーケストレータ側 harness に配る。home を持たない共有束であり、`inv_harness_skills … shared` とは呼ばない |
| `harnesses/<agent>/` | overlay instructions / skills / agents / prompts / commands / hooks / settings | 1 harness 専用、またはそのランタイム表面に密着する（同名で shared / agents を上書き可） |
...
| 実 dir + 項目ごと symlink（union） | skills / agents 等のコレクション | `~/.claude/skills/<name>` ← `agents/skills` + `harnesses/shared/skills` + `harnesses/claude/skills`（後勝ち） |
...
- **source 列は優先度低→高**。後から渡した source が同名を上書きする（harness skills: `agents/skills` < `harnesses/shared/skills` < `harnesses/<agent>/skills`）
