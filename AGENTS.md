...
- 例外: 外部ツール（orca / superset 等）が hooks を注入して書き換えるファイル（`~/.claude/settings.json` / `~/.cursor/hooks.json` 等）は symlink にしない。tracked 側に seed がある場合は `init.sh` が初回コピーし、意図的な設定変更は tracked 側にも手動で反映する
