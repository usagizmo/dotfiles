...
description: >-
  ...
  迷ったら使う（ユーザーが明示しなくても発動。短い確認・sanity check 含む）。実装プラン確定時だけ Plan Mode で GO を待つ。
  大きい diff の網羅レビューは review-loop。軽微・自明では使わない。
---

# 設計判断の突き合わせ（Claude / Codex）

**判断主体は Grok**（`procedure.md` の「実行中のエージェント」= Grok）。アドバイザーは検証者。
...
1. 仕様・変更対象・判断・スコープ外・検証を整理
2. `enter_plan_mode` → `plan.md` に書く → `exit_plan_mode` で承認 UI
3. GO まで実装しない（headless なら構造化 Markdown + 明示 GO）
