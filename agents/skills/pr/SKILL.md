...
name: pr
description: >-
  ブランチの実装・検証が完了したら PR を作成し auto-merge まで面倒を見る（タイトル先頭に gitmoji）。
  ユーザーが「PR を作成して」「pr 出して」と言ったとき、および仕上げ（commit / プロジェクトの verify 等）が済み積み残しが無いときに実行する。
---
...
- **積み残しの確認**: 当初の課題・プランに対して未実装・未解決のものがないか確認する。同スコープは追加実装。**今回変更由来の回帰・受け入れ失敗は Issue で消さず**直す / スコープ縮小 / 撤回。Issue 切り離しは既存不具合・要件外・独立改善に限る
