...
| 11  | **枠を渡す**                 | `runtime` が `待機` / `休止` かつ、待っているのが write なら**資源キーが write 保持者（対象集合自身は除く）のどれとも交差しない**（自身を除かないと、`実装中` で write を保持したまま `待機` に落ちた課題が**自分と交差して永久に枠を受け取れない** —— 人待ちから復帰する経路がこれ）、integration なら**渡しの記録を持っているか、記録がどこにも無くて次の受け手に選ばれている**（選び方は「資源」）                                                                                                                                                      | **休止の記録があるなら先に消してから**渡して再開させる。**integration は、渡す前に渡しの記録を書く**（記録が権限の実体。逆にすると、伝達が成功して記録が落ちたときに次の tick が別の課題へ渡す）                                                                |
