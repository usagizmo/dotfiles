...
| 交差を解消する / 休止を促し直す | **その課題の `runtime` が `休止` になった**（記録があり、かつ止まった）。**発火条件の否定にしない** —— 発火条件は「記録を持つ保持者が 0」なので、記録を書いた瞬間に偽になって **budget が一度も積まれない**。**2 つは同じ記録を伸ばす**（`lastAction` は「交差を解消する」） |
...
| 5   | **本文の変更を伝える**     | 対象集合のどれかの本文が計画の記録と違う。**かつ届け先のセッションが生きていて、`runtime` が `人待ち` でも `休止` でもない**                                                                                                                                                                                                                       | 再承認と再 plan が要ることを伝える（`references/protocols.md`）。**計画側が更新されるまで枠渡しと着地を止める**   |
