...
**自分の context が増える依頼を受け取らない。**問い合わせ・調査・文書仕事・skill の修正を渡されたら、
**着手せずに「自分の領分ではない」と 1 行返して終わる。行き先は書かない** —— 引き取り先を
知らなくても断れるし、**知る必要がある時点で層が逆さまになっている。**
...
（人が渡した回答の転記、指示された Issue の作成）、**どちらも他の節が明示的に許可している**。
2 つの述語を並べると、許可されている経路を「自分の領分ではない」と事実に反して断る。

**転記と、指示された Issue の作成は action ではない。**`1 tick 1 action` にも上限にも数えない
（`退避先` の `count` 精算・状況ボードの更新と同じ扱い）。転記のやり方は `references/harness.md`。
...
`count` 条件で降りるので、**pane を閉じる主体が居なくなって計画枠が永久に焼ける**）。
...
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 計画セッションを閉じる | `ledger` が `計画済み` 以降で、`refine-<番号>` のセッションが無い。**この行だけ発火条件の否定にしない** —— 否定形にすると pane を閉じた瞬間に解除され、**budget が一度も積まれない** |
...
実行器を動かす（8-10）→ 新しく始める（11-12）。**終わったものの片付けが台帳の前進より先**なのは、
...
| 3   | **計画セッションを閉じる** | `refine-<番号>` のセッションがあり、`runtime` が `人待ち` でなく、`ledger` が `未計画` でないか**そのセッションが動いていない**。**かつ `ledger` が `未計画` なら失敗の記録の `count` が上限に達していない**                                                                                                                         | pane を閉じる（計画枠を空ける）。**`ledger` が `未計画` のまま閉じるなら失敗の記録を進める**（下記）              |
...
| 9   | **解決を起こし直す**       | `progress` が `準備中`〜`着地待ち` かつ、`runtime` が `無し` / `人待ち` でセッションが無い。**かつ対象集合の台帳が揃っている**（揃っていなければ「台帳を進める」が先）。**checkout が無く、作ると容量が目安を超えるなら選ばない**（新しい checkout を作る action は 12 と同じ扱い）                                                  | checkout があるなら pane だけ。無いなら worktree を作ってから起こす                                               |
...
上限に達しても選ばれ続けると**差し戻しに永久に到達しない** —— pane を閉じられない harness では
