...
（失敗の記録を進め（`references/protocols.md`）、次の tick でやり直す。無言で次へ行かない）。
...
| 制約・異常 | `Conflict` の内容、容量が目安を超えていること、そのために選ばなかった action、**GitHub 観測不能・backoff 中**であること | 正規化レコードと容量、起床スナップショットの観測結果 |
