...
**人が `退避先` から戻したとき**。後者は Status の変更と `count: 0` を**一体で行う**。
戻さないと、復帰した課題が次の単発失敗で即座に `退避先` へ落ちる。

**1 つ目には例外がある。`lastAction` が「計画セッションを閉じる」の記録は、action の成功では
0 に戻さない**（理由は「action の優先順」）。**その記録を書いた「計画セッションを閉じる」自身の
成功も含む** —— 成功することが計上の理由なので、除かないと書いた同じ tick で消える。

**戻すのは現在の観測だけで決める。**`ledger` が `計画済み` 以降（`退避先` を除く）で、かつ
`refine-<番号>` のセッションが無いとき。**2 時点の比較にしない** —— tick は前回の続きを
仮定しないので、「前は `未計画` だった」を判定に使うと構造的に発火しない。
計画が前進すれば次の tick で両方が成立して自己修復し、閉じられないままならセッションが残るので
戻らない。

**2 つ目（人が `退避先` から戻したとき）に例外は掛からない** —— 掛けると `count` が上限のまま残り、
戻した次の tick で即座に `退避先` へ直帰して**人が救出できない**。
...
| **在庫が陳腐化した** | `ledger` が `計画済み` かつ `progress` が `未着手` かつ、在庫の記録が陳腐化している（判定は `references/ready-record.md`。**読めない・評価できないものは陳腐化**） | **未計画** | —（`ready` は次の `refine` が上書きする） |
...
**在庫 = 計画済みの group 数 + 稼働中の `refine-*` セッション数。**計画済みだけを数えると、
...
**数えるのは group であって Issue ではない**（1 group = 1 計画 = 1 branch = 1 write lease。
「同一ブランチ group」を参照）。**上限の数値を Issue の数と読まない** — 1 group は複数の Issue を
...
更新するのは**稼働中・詰まり・キュー・制約のいずれかが変わったとき**。変化の無い tick で書き直さない。
...
| 退避先の滞留 | 件数と、**古い数件を「いつから居るか」つきで**。人がここを見て未計画へ移すまで、どの action も当たらない | `ledger` が `退避先` の Issue と、その Status が変わった時刻 |
