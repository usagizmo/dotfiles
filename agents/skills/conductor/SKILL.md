...
| 10 | **枠を渡す** | `runtime` が `待機` かつ、待っているのが write なら**資源キーが write 保持者のどれとも交差しない**、integration なら**その課題が保持者として選ばれている** | 渡して再開させる |
...
| **write** | 同時書き込み。資源キーが交差しない | **課題** | `progress` が `実装中` / `提出中`、**または** `準備済み` かつ `runtime` が `稼働中` / `無し`。いずれも `ledger` が `退避先` でなく、**本文が計画の記録と一致している** | **数の上限を置かない**（資源キーの交差が決める） |
...
**判定は成果物（`progress`）で行い、セッションの見え方に依存させない。**`待機` は
「渡されるのを待っている」と「gate や人の答えを待って中断している」の両方に付き、
`runtime` だけでは切り分けられない。**`無し` を保持したままにしているのと同じ理由**で、
実体の状態ではなく成果物の有無で決める。
