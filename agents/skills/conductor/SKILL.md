...
（形と置き場所・読み取り側の定義はどちらも `references/same-branch.md`）。

**起票したら、並べ替えが要ることを人へ出す**。Project へ足す操作は**末尾に入れる**ことしかできず、
選出順は「他をブロックしている数 → ボード上の並び順」なので、**起票した Issue は人が動かすまで
事実上選ばれない**（実測で 205 件中 205 位）。出し先は状況ボードの「人がやること」。
...
| 9   | **解決を起こし直す**       | `progress` が `準備中`〜`着地待ち` かつ、`runtime` が `無し` / `人待ち` でセッションが無い。**かつ対象集合の台帳が揃っている**（揃っていなければ「台帳を進める」が先）。**checkout が無く、作ると容量が目安を超えるなら選ばない** —— ただし**その課題が論理 lease を保持しているなら、目安を超えても起こす**（新規の claim ではなく保持者の回収。控えると、保持者が容量の解放を待ち、容量を持つ課題が保持者の write 解放を待つ循環になる）                                                  | checkout があるなら入れ物だけ。無いなら worktree を作ってから起こす                                               |
| 10  | **交差を解消する**         | 資源キーが交差する write 保持者が 2 つ以上居て、**そのうち休止の記録を持つものが 1 つも無い**                                                                                                                                                                                                                                       | **後発**（`progress` が手前の方 → claim が新しい方 → 番号の順）に休止の記録を書いて伝える。**先発には何もしない**。worktree と変更は保持させる |
| 10b | **休止しない後発を報告する** | 休止の記録があるのにそのセッションが**動き続けている**                                                                                                                                                                                                                                                                             | 実行器を止める（`references/harness.md`）。**止められなければ `Conflict`**。ここが無いと、記録を書いた時点で 10 が偽になり 11 も動かず、**誰にも当たらないまま滞留する** |
| 10c | **checks を引き直させる**  | `progress` が `提出中` かつ `runtime` が `待機` かつ、**実行中の checks が 1 つも無い**                                                                                                                                                                                                                                             | 「誰も動いていない」という事実だけを伝える（**引き直し方は PR を持つ側に任せる**） |
| 11  | **枠を渡す**               | `runtime` が `待機` / `休止` かつ、待っているのが write なら**資源キーが write 保持者（対象集合自身は除く）のどれとも交差しない**（自身を除かないと、`実装中` で write を保持したまま `待機` に落ちた課題が**自分と交差して永久に枠を受け取れない** —— 人待ちから復帰する経路がこれ）、integration なら**その課題が保持者として選ばれている** | **休止の記録があるなら先に消してから**渡して再開させる |
...
**`ledger` が `退避先` の課題に当てるのは、上 3 つ**（報告して止める・片付ける・計画セッションを片付ける）**だけ**。人が見るまで
...
remote branch、PR（state・checks）、固定 marker の 6 記録（claim・計画・在庫の鮮度・人待ち・休止・失敗）、セッション、
...
| **integration** | **merge**。本当の直列化点          | **課題**   | `着地待ち` かつ `runtime` が `人待ち` でなく `ledger` が `退避先` でなく、**本文が計画の記録と一致している**もののうち、PR 作成が最も早い 1 件（**`休止` は足さない**。下記） | **常に 1**（変更不可）                             |
...
**`休止` を integration の保持条件に足さない。**yield は write の交差でしか起きず、`着地待ち` では
write が既に空いている（`提出中` → `着地待ち` で空く）。**足さない理由を書いておかないと、
「対称にすべき」で後から足されて merge が止まる。**
...
1. **先に write を取った側が継続する**（先発 = `progress` が進んでいる方 → claim が古い方 → 番号の順）
2. 後発は安全なチェックポイントで休止する。**休止の記録は conductor が書き、conductor が消す**
3. **worktree と変更は保持する**（破棄しない）
4. 先行が着地したら rebase・再計画して再開する

**「後発」を claim の新しさだけで決めない。**実測で、**PR 目前まで進んだ課題を止めて、
まだ 1 行も書いていない課題を continue させた**。進捗を先に見れば、捨てる作業量が最小になる。
...
| 制約・異常   | `Conflict` の内容、容量が目安を超えていること、そのために選ばなかった action、**GitHub 観測不能・backoff 中**であること、**`退避先` へ落ちた課題と原因の `lastAction`**（retry budget 切れで落ちたものだけ。人が置いたものには記録が無い）、**宣言に見えるのに読まれない出現**（下記）、**この tick で自分の規約を曲げたこと・規約の穴に気づいたこと**（下記） | 正規化レコードと容量、起床スナップショットの観測結果、失敗の記録、Issue 本文、この tick の実行 |
...
**`retired-refine-<番号>` が残っている Issue と、この tick で起票した Issue も出す。**前者は
**人が pane を閉じるまで再計画されない**、後者は**人がボードを並べ替えるまで選ばれない**
（起票は末尾に入り、選出順は「ブロック数 → ボード順」なので事実上いつまでも回ってこない）。
どちらも「異常が起きた」形では現れないので、出さなければ黙って止まる。
...
**URL は記憶しない。題名を固定し、一覧から引き当てる**（無ければ新規に作る）。tick は前回の
続きを仮定しないので、覚えた URL は次の tick には**正しい保証が無い記憶**になる。それでも掴むと、
**作り直された版を見失って古い方を更新し続ける** —— 人が一覧から開くのは新しい方なので、
**更新しているのに盤面が止まって見える**（読むときも同じ理由で題名から引き当てる）。
tick ごとに新しい Artifact を作らない。
