...
| 12 | **claim する** | 選出の条件をすべて満たす。**かつ作っても容量が目安を超えない**（資源。目安 5 なら既に 5 ある時点で選ばない） | branch を作って起こす（下記） |
...
| --- | --- | --- | --- | --- |
| **容量** | ディスクと build 資源 | worktree | `capacity` が `あり` | **目安 5**（超過可） |
...
**作ると目安を超えるなら claim しない**（claim 手順 5 で worktree を作るため。目安 5 なら
既に 5 ある時点で選ばない）。
自分から増やさないだけで、超過そのものは**外から起きる**（人が直接 `resolve` を走らせた等）。
**そのときも通常どおり動く** — 超過を Conflict にせず、既存の課題を進める action は選び続ける。
...
| 制約・異常 | `Conflict` の内容、容量が目安を超えていること、そのために選ばなかった action | 正規化レコードと容量 |
