...
| 10 | **枠を渡す** | `runtime` が `待機` かつ、待っているのが write なら**資源キーが稼働中のどれとも交差しない**、integration なら**その課題が保持者として選ばれている** | 渡して再開させる |
...
| **write** | 同時書き込み。資源キーが交差しない | **課題** | `progress` が `準備済み`〜`提出中` かつ `runtime` が `稼働中` / `無し` かつ `ledger` が `退避先` でなく、**本文が計画の記録と一致している** | **数の上限を置かない**（資源キーの交差が決める） |
