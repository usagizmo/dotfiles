...
**セッションの生死しか外からは見えない**（状態値とその意味は `references/harness.md`）。
...
| `HEAD` が default と**同一 SHA** かつ **clean**、計画コメントが無い | prepare 中 | read |
| 同上で**計画コメントがある** | prepare 済み（write 待ち） | read |
...
**表の工程は「どこまで進んだか」で、保持 lease はその既定値。**
...
**lease はこの表から復元する。観測できる粒度は `resolve` の工程より粗い** — 仕上げ・検証・
意図の確認は worktree からは実装中と区別できない。共通する工程は同じ語で呼ぶ（状況ボードに出るため）。
**integration 待ちが複数あるときは PR 作成が早い方が lease を持つ** — 表の列だけ見て全員が
持っていると数えない。**状態表示だけで完了と判定しない。**

**idle は 3 通りある。人待ちを先に除く**（残りを工程で引く）。

| 観測 | idle の意味 | すること |
| --- | --- | --- |
| 人待ちの印がある（`references/harness.md`）、または `resolve-` のセッションで Status が**計画済み**へ戻っている | **人待ち** | lease を返す。詰まりとして出す |
| 工程が **prepare 済み** / **integration 待ち** | lease 待ち | 空いたら再開させる |
| どちらでもない | **中断** | 同じセッションを再開させる |

人待ちは 3 形態ある。**Status だけでは引けない** — `resolve` の停止条件は計画済みへ戻すが、
`refine` の質問待ちは未計画のまま、`resolve` の意図の確認は進行中のまま止まる。
残り 2 つは印でしか引けないので、Status を先に見て終わらせない。
...
flowchart TD
    ...
    L -->|write lease| P2[prepare 済み / 人待ち復帰を再開させる]
...
集合とその計画（`resourceKeys`）が貸出状態そのもの。**ただし枠を握っているのは worktree** —
工程の判定表がそこから lease を復元するので、セッションだけが死んでも枠は空かない。
**空ける経路は「stale の回収」の表だけ**で、明示的な解放の代わりにこれが要る。

**人待ちは lease を返す。**製品判断を待って止まっているセッションは実行資源を使っていないので
稼働中に数えない。数えると、答えを待っている間だけ並列度が落ちる。判定は「工程の判定」の idle 表。

**答えが返っても、そのまま書かせない。**返した枠は別の課題が使っている可能性があるので、
**人待ちから復帰したセッションは lease を取り直す。**交差していれば「予定範囲を超えたとき」の
後発として休止させる。取り直さないと、資源キーが交差する 2 つが同時に書く。

代わりに**人待ちの総数に上限を置く**（既定 3）— 答えられる量を超えて質問を積んでも、
待ち行列が伸びるだけで速くならない。**達したら `refine` を新たに起こさない。**質問を生むのは
計画工程なので、`resolve` の消化と再開は止めない（止めると write 枠が空いていても全停止する）。
...
**戻し先を混ぜない。**人待ち・stale を未計画へ戻さない — refine がすぐ拾い直すので、**止まらない。**
...
| 計画が無効（前提が崩れた・**Issue 契約が欠けている**） | **未計画**。refine からやり直す |
...
**揃っていない group の残りを計画する refine は、在庫・人待ちどちらの上限も超えて起こす。**上限の目的は
...
- その Issue 番号の remote branch がある（**worktree もセッションも無いものは除く** — 実体が
  無い branch は「stale の回収」が起こし直すので、選出で拾うと二重になる）
...
**3 が失敗したらセッションを起こさない。**`resolve` は Status が進行中であることで `managed` と
判定するので、台帳がずれたまま起こすと **lease を一度も待たない `interactive` として着地まで走る**
...
| 台帳だけずれている | branch はあるが Status が進行中でなく、**worktree もセッションも無い** | **Status を直す**（claim は生きている）。直してから起こす |
| **セッションだけ死んだ** | 進行中で **worktree はあるが、セッションが無い** | **同じ worktree で起こし直す。**実装は破棄しない |
| **実体が消えた** | 進行中で branch はあるが、**worktree もセッションも無い** | **worktree を作り直して起こす**（branch に commit があればそこから続く） |
| `refine` が終わっている | `refine-<番号>` のセッションがあるのに Status が**未計画でない** | **pane を閉じる**（read 枠を空ける） |
| 実体だけ残っている | 工程の判定表で**完了**なのに worktree / セッションがある | **稼働中に数えない。**自分が作ったものは片付ける |
| worktree が prunable | checkout が消えている | 稼働中に数えない。自分が作ったものなら片付ける |

完了かどうかは工程の判定表に従う。起こし直し・片付けの手順は `references/harness.md`。

**回収は起こし直しまで含む。**表で「起こす」と書いた行はこのフェーズで実行し、action に数える
（次のフェーズの選出では拾えない — claim 済みの branch があるので選出の条件で弾かれる）。

**実体が残っている限り、lease は解放されない。**工程の判定表は worktree から lease を復元するので、
セッションが死んでも枠は空かない。**この表だけが枠を空ける経路**なので、tick で必ず通す。
...
| 詰まり | 人待ちのもの。**何を答えれば進むか**を 1 行で | Issue のコメント、または人待ちセッションの最後の応答 |
