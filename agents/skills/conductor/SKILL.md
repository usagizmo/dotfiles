...
| 3 | **計画セッションを閉じる** | `refine-<番号>` のセッションがあり、`runtime` が `人待ち` でなく、`ledger` が `未計画` でないか**そのセッションが動いていない** | pane を閉じる（read を空ける） |
...
| 8 | **計画を起こし直す** | `ledger` が `未計画` かつ `runtime` が `人待ち`（記録が `waiting`）で、計画セッションが無い。かつ **read に空きがある** | pane だけ作って起こす（worktree は使わない） |
| 9 | **解決を起こし直す** | `progress` が `準備中`〜`着地待ち` かつ、`runtime` が `無し` / `人待ち` でセッションが無い。**かつ対象集合の台帳が揃っている**（揃っていなければ「台帳を進める」が先）。かつ **read に空きがある。checkout が無いなら容量にも空きがある** | checkout があるなら pane だけ。無いなら worktree を作ってから起こす |
| 10 | **枠を渡す** | `runtime` が `待機` かつ、待っているのが write なら**空きがある**、integration なら**その課題が保持者として選ばれている** | 渡して再開させる |
| 11 | **計画を起こす** | `ledger` が `未計画` かつ `progress` が `未着手` かつ計画セッションが無く、人待ちの記録が `waiting` **でない** | `refine` を起こす（在庫と人待ちの上限を見る） |
...
**起こし直された側は、まず人待ちの記録を読む**（`waiting` なら質問を出し直して待つ。人が既に
答えていれば解釈して `cleared` にしてから続ける）。記録を書いた当人が死んでいても、ここで
復帰経路が閉じる。
...
矛盾する。**論理 lease（write / integration）の保持者は課題**、**物理枠（容量 / read）の保持者は
実体**、と分けると両立する。

| 資源 | 何を守るか | 保持者 | 保持している条件 | 同時数 |
| --- | --- | --- | --- | --- |
| **容量** | ディスクと build 資源 | worktree | `capacity` が `あり` | 9 |
| **read** | 同時実行器の数 | セッション | **生存しているセッション**（`refine-*` / `resolve-*`。人待ちでも返さない） | 3 |
| **write** | 同時書き込み。資源キーが交差しない | **課題** | `progress` が `準備済み`〜`提出中` かつ `runtime` が `稼働中` / `無し` かつ `ledger` が `退避先` でなく、**本文が計画の記録と一致している** | 4 |
...
**`待機` は非保持。**渡されるのを待っている状態なので、記録を `cleared` にしただけでは戻らない
（`resolve` 側の「渡し直されるまで書き始めない」と同じことを、観測側から言っている）。
**`無し` は保持したまま** — 実装は残っているので、起こし直すまで枠を空けない。
...
**物理枠は実体そのものを数える。**`capacity` が `あり` の数が容量、**生存セッション数**が read。
read は人待ちでも返らない（実行器は残っているので）。**空くのはセッションが消えたときだけ** —
計画セッションを閉じる行が要るのはこのため。

**容量が上限なら、write が空いていても claim しない**（claim 手順 5 で worktree を作るため）。
`refine` は worktree を作らないので、容量を使うのは `resolve` だけ。read は両方が使う。
...
**答えられる量を超えて質問を積んでも、待ち行列が伸びるだけで速くならない。**`waiting` の記録を
group 単位で数え、上限（既定 3）に達したら **`refine` を新たに起こさない**。

**これは起こしてよいかの閾値であって、稼働中を止める上限ではない。**質問を生むのは計画工程なので、
既に動いている `resolve` は止めない（止めると write 枠が空いていても全停止する）。
稼働中の `resolve` が追加で止まれば 3 を超えるが、それは正しい挙動。
