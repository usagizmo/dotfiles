...
| 9 | **解決を起こし直す** | `progress` が `準備中`〜`着地待ち` かつ、`runtime` が `無し` / `人待ち` でセッションが無い。**かつ対象集合の台帳が揃っている**（揃っていなければ「台帳を進める」が先）。**checkout が無く容量が目安を超えているなら後回し**（他に選べる action が無ければ進めてよい） | checkout があるなら pane だけ。無いなら worktree を作ってから起こす |
...
**容量だけは目安で、超えても止めない。**ディスクと build 資源は実際に尽きるまで分からないので、
**数で fail-closed にせず、超えている間は新しい checkout を伴う action を後回しにする**（既存の
課題を進める action を先に選ぶ）。それでも進める必要があるなら作ってよい。**超過している事実は
状況ボードに出す。**暴走を止めるのは 1 tick の action 数と在庫であって、容量ではない。
...
**容量が目安を超えているなら claim を後回しにする**（claim 手順 5 で worktree を作るため）。
他に選べる action が無いなら claim してよい。
