...
**`退避先` へ落とすとき**。後者は Status の変更と `count: 0` を**一体で行う**
（片方だけ成功すると `count` が上限のまま `退避先` に残り、救出した次の tick で直帰する）。

**戻す主体を人にしない。**Status を動かすのは人だが、**人は失敗の記録を書き換えない**。
「人が戻したときに 0 にする」と書くと**実行する主体がどこにも居らず**、救出した次の tick で
差し戻しの述語（`count` が上限）が**新しい失敗を 1 回も要さずに真のまま**当たって直帰する。
落とす側がその場で精算すれば、この往復は起きない。**`lastAction` は残す**（原因は状況ボードが読む）。

**1 つ目には例外がある。次の `lastAction` を持つ記録は、action の成功では 0 に戻さない。**

| `lastAction` | 例外の解除条件（現在の観測だけで決める） |
| --- | --- |
| 計画セッションを閉じる | `ledger` が `計画済み` 以降（`退避先` を除く）で、`refine-<番号>` のセッションが無い |
| 本文の変更を伝える | 計画の記録の本文 digest が**現在の本文と一致した** |
| 計画の失効を伝える | 計画の `baseSha..default` が `invalidationScope` にも `resourceKeys` にも**交差しなくなった** |
...
**2 つ目（`退避先` へ落とすとき）に例外は掛からない。**

**tick の終わりに状況ボードの更新判定を行う**（条件は「状況ボード」の節）。
...
| 3 | **計画セッションを閉じる** | `refine-<番号>` のセッションがあり、`runtime` が `人待ち` でなく、`ledger` が `未計画` でないか**そのセッションが動いていない**。**かつ失敗の記録の `count` が上限に達していない** | pane を閉じる（計画枠を空ける）。**`ledger` が `未計画` のまま閉じるなら失敗の記録を進める**（下記） |
...
| 10 | **枠を渡す** | `runtime` が `待機` かつ、待っているのが write なら**資源キーが write 保持者（対象集合自身は除く）のどれとも交差しない**（自身を除かないと、`実装中` で write を保持したまま `待機` に落ちた課題が**自分と交差して永久に枠を受け取れない** —— 人待ちから復帰する経路がこれ）、integration なら**その課題が保持者として選ばれている** | 渡して再開させる |
| 11 | **計画を起こす** | `ledger` が `未計画` かつ `progress` が `未着手` かつ計画セッションが無く、人待ちの記録が `waiting` **でない** | `refine` を起こす（在庫の上限と計画枠を見る） |
| 12 | **claim する** | 選出の条件をすべて満たす。**かつ作っても容量が目安を超えない**（資源） | branch を作って起こす（下記） |
...
**作ると目安を超えるなら claim しない**（claim 手順 5 で worktree を作るため）。
...
| retry budget 切れ | 失敗の記録の `count` が上限に達した | **退避先** | **`count` を 0 に戻し、`lastAction` は残す**（契機と理由は「1 tick」） |
| 計画が無効 | **Issue 契約が欠けている**かつ **commit も dirty も無い** | **未計画** | **claim の記録を消し、branch と実体を片付ける** |
| **在庫が陳腐化した** | `ledger` が `計画済み` かつ `progress` が `未着手` かつ **`Depends on` がすべて解消している**かつ、在庫の記録が陳腐化している（判定は `references/ready-record.md`。**読めない・評価できないものは陳腐化**） | **未計画** | —（`ready` は次の `refine` が上書きする） |
...
**在庫 = 計画済みの group 数 + `refine-*` セッション数**（数え方は計画枠と同じ —— **生存していて
`runtime` が `人待ち` でないもの**）。**`稼働中` だけを数えない** —— `待機` の `refine` が漏れ、
下の穴が部分的に再開する。計画済みだけを数えると、
...
更新するのは**下の表のどれかの行が変わったとき**。変化の無い tick で書き直さない。
...
| 依存待ち | `Depends on` が未解決のもの。**何が着地すれば動くか**を 1 行で | Issue 本文の `Depends on` と依存先の open / closed |
| 退避先の滞留 | 件数と番号。人がここを見て未計画へ移すまで、**どの工程も起きない**（`refine` / `resolve` を起こす action が当たらない） | `ledger` が `退避先` の Issue |
| 制約・異常 | `Conflict` の内容、容量が目安を超えていること、そのために選ばなかった action、**GitHub 観測不能・backoff 中**であること、**`退避先` へ落ちた課題と原因の `lastAction`**（retry budget 切れで落ちたものだけ。人が置いたものには記録が無い） | 正規化レコードと容量、起床スナップショットの観測結果、失敗の記録 |
