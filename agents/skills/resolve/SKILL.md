...
| **提出** | latest default へ追随して `pr`。**CI が緑になるまでここ** |
...
**複数番号を渡されたら、そのどれか 1 つで branch が見つかれば claim 済み。**1 本のブランチで
複数の Issue を片付けるとき branch 名に入るのは代表の番号だけなので、全番号に branch を求めると
`interactive` に落ちる。
...
人を待つのは次の 4 つだけ。当たったら **Status を計画済みへ戻し**（答えが返れば同じ計画で
再開できる）、理由を Issue にコメントする。
...
**分かれ目は受入条件を満たしたかどうか。**満たす前に気づいたものは、テーマが違っても
別の場所でも**そのブランチで直す**。今回変更由来の回帰は満たしていないので常にこちら。
...
| **製品境界を含む**（定義は「停止条件（managed）」の節。定義自体は variant に依らない） | Issue にせず、セッションまとめに「未対応 + 理由 + 推奨」として残す |
