...
**原則 `consult` を回さない。**「何を作るか」は Issue に label が付いた時点で決着している
（label = 計画済みの宣言）。ここで出すのは「どう作るか」— 触る範囲・資源キー・無効化条件だけ。
...
ユーザーが分けると決めたものだけ `issue` で Issue にし、**別 worktree + 別セッションでその Issue を解決する**（Issue を後回しのキューにしない）。worktree / セッションの起こし方は harness 側に従う。
