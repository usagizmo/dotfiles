...
応答・Issue / PR 本文・commit message・docs など、agent が出す文章すべてに適用する。

- **強調の内側の端に全角約物を置かない** — `。、」』）` は `**` の外へ出す。`**…ない**。` は成立し、`**…ない。**` は成立しない。CommonMark の flanking 規則では、閉じ `**` の直前が約物で直後が通常文字だと閉じ記号と認められず、GitHub 等で `**` がそのまま表示される。開き側も同じで `する**「A」**は` は壊れる（直後が約物・直前が通常文字）
- 内側の端に約物が要るなら、`**` の外側に半角スペースを置くか `<strong>` を使う
